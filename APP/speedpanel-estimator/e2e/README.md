...
- **`e2e/*.spec.ts`** (Playwright) -- signs in through the real UI as each
  persona and exercises pages, direct-fetch bypass attempts, and Edge
  Function auth. **Written and smoke-tested for structural correctness**
  (launches, navigates, fails at the expected point), but **not verified
  passing end-to-end** -- see "What's actually verified" below.
...
Fixture data: two companies (A, B); 4 projects across
draft/install_review/technical_review stages; 3 orders across
...
npm run dev             # or point PLAYWRIGHT_TEST_BASE_URL at a deployed instance
npm run test:e2e
```
Tear down with `supabase/teardown-e2e.sql` (deletes every seeded row by
`@e2e.test` email / fixed UUID) whenever you're done.

## What's actually verified (this session)

- ✅ **Part B (seed)**: all 10 accounts + all fixture rows created and
  confirmed present; all 10 passwords confirmed to authenticate against
  their stored hash.
- ✅ **Part C (pgTAP/SQL-RLS)**: all 36 assertions across the 4 test files
  run for real against the live project via the Supabase MCP connector's
...
- ⚠️ **Part D (Playwright)**: written completely, typechecks cleanly, and
  was smoke-tested against this environment's pre-installed Chromium. Two
  pure-UI assertions (no network call) passed. Every assertion requiring a
  real Supabase Auth/REST round-trip failed with `Failed to fetch` --
  **this sandbox's network policy blocks outbound HTTPS to `*.supabase.co`
  from a browser context**, confirmed via the proxy's own denial log
  earlier in this session. This is a sandbox limitation, not a defect in
  the app or the test suite -- `npm run test:e2e` needs to be run somewhere
  with real network access (a developer machine or CI) to get a genuine
  pass/fail result.
- ⬜ **Part E (local Docker path)**: `supabase/config.toml` was generated by
  ...
  Postgres/GoTrue/Kong container images (confirmed via the same proxy
  denial log). Untested, but should work in a normal Docker-capable
  environment.
