...
```
supabase start
supabase db reset      # applies schema.sql + seed.sql (config.toml's
                        # [db.migrations] schema_paths now points at
                        # ./schema.sql, so this actually loads it)
...
- ✅ **pgTAP/SQL-RLS**: all 36+ assertions across the test files run for
  ...
  stack on every PR/push -- this required fixing `config.toml`'s
  `[db.migrations] schema_paths`, which was previously empty (`[]`) and so
  `supabase db reset` never actually applied `schema.sql` despite this
  doc's Path A instructions claiming it did.
...
- ⬜ **Local Docker path (Path A)**: `supabase/config.toml` was generated by
  ...
  `seed.sql`, and `[db.migrations] schema_paths` now points at
  `./schema.sql` too, but `supabase start`/`supabase db reset` themselves
