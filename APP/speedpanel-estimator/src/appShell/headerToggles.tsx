...
export const LayoutModeToggle = ({ effective, onToggle }: { effective: EffectiveLayout; onToggle: () => void }) => (
  <IconButton size="header" onClick={onToggle} title={effective === "phone" ? "Layout: Phone (click for Web)" : "Layout: Web (click for Phone)"}>
...
export const ThemeToggle = ({ effective, onToggle }: { effective: EffectiveTheme; onToggle: () => void }) => (
  <IconButton size="header" onClick={onToggle} title={effective === "dark" ? "Theme: Dark (click for Light)" : "Theme: Light (click for Dark)"}>
...
export const NotificationBell = ({ count, onClick }: { count: number; onClick: () => void }) => (
  <span className="group relative inline-block">
    <IconButton size="header" onClick={onClick} title={count > 0 ? `${count} open request${count !== 1 ? "s" : ""}` : "My Requests"}>
      <Bell size={16} />
    </IconButton>
    {count > 0 && (
